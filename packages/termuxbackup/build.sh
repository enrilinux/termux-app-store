TERMUX_PKG_HOMEPAGE=https://github.com/enrilinux/termuxbackup
TERMUX_PKG_DESCRIPTION="Termux TUI backup tool"
...
termux_step_make() {
    ...
    if grep -qE "^termuxbackup[[:space:]]*:" "$_mk_file" 2>/dev/null; then
        _mk_target="termuxbackup"
...
termux_step_make_install() {
    ...
    for _candidate in "$_mk_binary" "termuxbackup" "termuxbackup.out" "termuxbackup-bin"; do
    ...
    if [[ -n "$_bin" ]]; then
        ...
        install -Dm755 "$_bin" "$TERMUX_PREFIX/bin/termuxbackup"
        echo "[ OK ] Installed: $TERMUX_PREFIX/bin/termuxbackup"
    else
